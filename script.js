...
function checkCards(amount) {
  let imgCards = [...document.getElementsByClassName("card-template")];
  console.log(imgCards.length,amount);

  console.log('Checking if cards loaded...', imgCards.length)
  if (imgCards.length !== amount) {
    setTimeout(checkCards, 1000, amount);
  } else {
    loadCards(imgCards);
  };
}

function loadCards(imgCards) {
  console.log("Loading card function running...")
  imgCards.map(function (element, index) {
    const cardData = {
